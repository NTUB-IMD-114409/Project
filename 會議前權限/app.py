...
from db import get_db, insert_user, get_user_by_email, delete_organization, get_organization_members, get_user_by_id, delete_org_member_by_email, update_meeting_member_role_db, update_user_password
...
model = WhisperModel("base", device="cpu") # 初始化 Whisper 模型
...
def upload_file():
    ...
    if not row or row["role"] not in ["主持人", "可供編輯"]:
