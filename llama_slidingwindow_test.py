...
# ===== 問答主功能（sliding window 多段問答）=====
CHUNK_SIZE = 300
STRIDE = 200
MAX_TOKENS = 480  # 預留模型回答空間
...
def count_tokens(text):
    # 中文約 1 token / 1.3 字，英文平均 3~4 字 = 1 token
...
def ask_file():
    try:
        ...
        # === 3. 切段成多個滑動視窗 ===
        chunks = sliding_window_chunks(file_content, CHUNK_SIZE, STRIDE)
        all_answers = []

        # === 4. 對每段逐一問答 ===
        for idx, chunk in enumerate(chunks):
            prompt = f"以下是會議記錄片段，請根據內容回答問題：\n{chunk}\n\n問題：{question}"

        # ✅ 新增這段：避免 token 超過模型限制
            token_count = count_tokens(prompt)  # ✅ 自己定義的
            if token_count > MAX_TOKENS:
                print(f"⚠️ 跳過第 {idx+1} 段（{token_count} tokens）超過限制")
                continue
        ...
        # === 5. 整合所有片段回答 ===
        merged_text = "\n".join(all_answers)
        merge_prompt = f"以下是根據逐段會議記錄所產生的回答，請統整成清楚一致的回覆：\n{merged_text}"

        final_response = get_llm().create_chat_completion(
        ...
        # === 6. 儲存 QA 紀錄進資料庫 ===
        conn = get_db()
        insert_qa_log(conn, meeting_id, user_id, question, final_answer)
        conn.close()
