...
formal_doc_bp = Blueprint("formal_doc_bp", __name__)
...
def ai_generate_meeting_doc(content, headings, output_name, template_id, ref_filename):
 ...
 # === 從 md_text 抓「待辦事項」 ===
    ...
    # === 改成只回傳分析文字（不轉成 Word） ===
    # 🧩 依 heading 分段（方便前端顯示在不同編輯框）
    sections = []
    for h in headings:
        # 嘗試找每個標題的內容
        part = re.search(rf"#\s*(?:🏫|📅|⚖️)?\s*{h}([\s\S]*?)(?=\n#|$)", md_text)
        sections.append({  
            "heading": h,
            "content": part.group(1).strip() if part else ""
        })
...
# === 生成正式文件草稿（回傳前面 AI 分析的內容給前端顯示） ===
@formal_doc_bp.route("/generate_formal_doc_draft", methods=["POST"])
def generate_formal_doc_draft():
    try:
        data = request.json
        ...
        if err:
            return jsonify({"status": "error", "message": f"讀檔失敗：{err}"}), 400

        # 找模板
        template = next((t for t in TEMPLATES if str(t["id"]) == str(template_id)), None)
        if not template:
            return jsonify({"status": "error", "message": f"找不到模板（ID: {template_id})"}), 400

        # 🧠 呼叫 GPT 分析函式（內部函式，不是 route）
        result = ai_generate_meeting_doc(content, template["headings"], "draft_preview", template_id, ref_filename)

        return jsonify(result)

    except Exception as e:
        logger.exception("產生文件草稿失敗")
...
def generate_formal_doc():
    try:
        ...
        # ✅ 新增：接收前端修改後的內容（sections）
        sections = data.get("sections", [])  # ← 新增這行
        ...
        if sections:
            ...
            logger.info(f"📄 使用前端編輯內容生成正式文件，共 {len(sections)} 段")
        else:
            # 沒有 sections 時，退回使用原始 AI 分析內容
            context = content
            logger.info("📄 使用原始 AI 分析內容生成正式文件")
        ...
        logger.info(f"正式文件已生成：{out_file}，待辦事項數量：{len(tasks)}")
        ...
        try:
            ...
            cursor.execute(
                """
                INSERT INTO files (meeting_id, file_name, file_path, file_type, uploaded_by, uploaded_at)
                VALUES (%s, %s, %s, %s, %s, %s)
                """,
                (meeting_id, output_filename, rel_path, "會議正式文件", uploaded_by, datetime.now())
            )
