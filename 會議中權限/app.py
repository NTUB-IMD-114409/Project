...
from db import get_db, insert_user, get_user_by_email, delete_organization, get_organization_members, get_user_by_id, delete_org_member_by_email, update_meeting_member_role_db, update_user_password, add_file
...
import requests  # ✅ 新增，用來呼叫中研院的 LLaMA API
from flask import request, jsonify
UPLOAD_FOLDER = "uploads"  # ✅ 記得放在 app.py 的最上面
LLAMA_API_URL = "http://140.109.23.191:8001/llama"
# source venv/bin/activate

app = Flask(__name__)

# 初始化 Whisper（用 CPU 或 GPU）
...
model = WhisperModel("base", device=device) # 初始化 Whisper 模型
...
def meeting_member_page(meeting_id):
    ...
    return render_template("meeting.before/meeting_member.html", meeting_id=meeting_id, role=role)
...
def whisper_transcribe():
    try:
        ...
        # 改成絕對路徑
        full_audio_path = os.path.abspath(os.path.join("uploads", filename))
        base_filename = os.path.splitext(filename)[0]
        output_txt_path = os.path.abspath(os.path.join("output", base_filename + ".txt"))

        # 確保檔案存在
        if not os.path.exists(full_audio_path):
            return jsonify({"error": f"音檔不存在：{full_audio_path}"}), 404

        # Whisper CLI
        whisper_cmd = f'whisper "{full_audio_path}" --model base --language Chinese --output_dir output --output_format txt'
        ...
        summary_ids = bart_model.generate(inputs["input_ids"], max_length=512, num_beams=4, early_stopping=True)
        ...
        docx_output_path = os.path.join("output", f"{base_filename}.docx")
        doc.save(docx_output_path)

        return jsonify({
            "result": transcript,
            "summary": summary,
            "download_url": f"/static_download/{os.path.basename(docx_output_path)}"
        })

    except subprocess.CalledProcessError as e:
        return jsonify({"error": f"Whisper CLI 執行失敗：{str(e)}"}), 500
    except Exception as e:
...
def ask_llama(context, question):
    prompt = f"""
你是一位專業會議助理，請根據以下會議資料，**只回答使用者的提問內容**，請勿額外補充未被詢問的資訊，並使用繁體中文作答：
...
"""
    try:
        ...
        return response.json().get("result", "⚠️ 回傳格式異常，請檢查 API。")
    except Exception as e:
        print("❌ LLaMA API 錯誤：", e)
        return "⚠️ 查詢失敗，請稍後再試。"

# ✅ 接收使用者問題與檔案路徑，讀取檔案後送給 LLaMA
@app.route("/api/ask_file", methods=["POST"])
def ask_file():
    data = request.get_json()
    relative_path = data.get("file_path")  # 如 meeting_67/meeting.docx
    question = data.get("question")

    if not relative_path or not question:
        return jsonify(success=False, message="請提供檔案路徑與問題"), 400

    try:
        full_path = os.path.join(UPLOAD_FOLDER, relative_path)  # ✅ 組出實體路徑
        context = load_file_text(full_path)  # ✅ 用絕對路徑讀取
        answer = ask_llama(context, question)
        return jsonify(success=True, answer=answer)
    except Exception as e:
        return jsonify(success=False, message=str(e)), 500
    
    print("👉 接收到的 file_path:", relative_path)
    print("👉 問題內容：", question)
