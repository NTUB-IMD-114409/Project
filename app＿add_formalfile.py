...
def upload_file():
    ...
    if file and allowed_file(file.filename):
        ...
        if filename.lower().endswith((".docx", ".pdf")):
            ...
            try:
                build_doc_embeddings(save_path, pkl_path)
                print(f"✅ 已建立 embedding 檔：{pkl_path}")
            except Exception as e:
...
def delete_file():
    ...
    try:
        ...
        if os.path.exists(file_path):
            os.remove(file_path)
        pkl_path = file_path + ".pkl" # 刪除.pkl embedding 檔案
...
def whisper_transcribe():
    try:
        ...
        print(f"📌 接收到 filename: {filename}, use_basic: {use_basic}")
        ...
        polished_text = auto_polish_text(traditional_text, use_basic=use_basic)
...
def generate_docx():
    try:
        ...
        if template_id != 1:
            return jsonify({"success": False, "error": "目前只支援 template_id=1"}), 400
        ...
        if not meeting:
            return jsonify({"success": False, "error": "找不到該會議"}), 404

        # 🔧 建立 context 對應 Word 模板欄位
        context = {
            "university": meeting.get("university", "國立示範大學"),
            "year": meeting.get("year", "113"),
            "semester": meeting.get("semester", "第一"),
            "meeting_number": meeting.get("meeting_number", "3"),
            "meeting_name": meeting.get("meeting_name", "教務會議"),
            "month": meeting.get("month", "7"),
            "day": meeting.get("day", "31"),
            "weekday": meeting.get("weekday", "三"),
            "time": meeting.get("time", "10:00"),
            "campus": meeting.get("campus", "校本部"),
            "floor": meeting.get("floor", "行政大樓"),
            "room": meeting.get("room", "第一會議室"),
            "chairman": meeting.get("chairman", "張三"),
            "expected": meeting.get("expected", "15"),
            "recorder": meeting.get("recorder", "李四"),
            "actual": meeting.get("actual", "13"),
            "last_resolution": meeting.get("last_resolution", "無異議通過"),
            "chairman_reports": meeting.get("chairman_reports", ["略"]),
            "committee_reports": meeting.get("committee_reports", ["教學委員會報告一", "學生事務委員會報告二"]),
            "committee_comments": meeting.get("committee_comments", ["加強課程整合", "鼓勵多元參與"]),
            "proposals": meeting.get("proposals", [
                {"subject": "新增人工智慧課程", "department": "教務處", "explanation": ["提升學生能力", "與業界接軌"], "resolution": "通過"},
                {"subject": "更換老舊電腦", "department": "總務處", "explanation": ["提高設備效率"], "resolution": "修正後通過"}
            ]),
            "emergency_items": meeting.get("emergency_items", [
                {"title": "期末教學回饋調查", "content": "將於下週一開放填寫"}
            ]),
            "end_time": meeting.get("end_time", "上午 11:40")
        }

        tpl = DocxTemplate("templates/templates_docx/uni_meeting.docx")
        tpl.render(context)
