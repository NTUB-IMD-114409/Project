...
def create_meeting():
    ...
    try:
        ...
        cursor.execute(
            """
            INSERT INTO meetings (title, date, org_id, topic_id, created_by, status, created_at)
            VALUES (%s, %s, %s, %s, %s, %s, NOW())
            """,
            (title, date, org_id, topic_id, creator_id, "before")
        )
        ...
        for p in participants:
            ...
            if user:
                cursor.execute(
                    "INSERT INTO meeting_participants (meeting_id, user_id, role) VALUES (%s, %s, %s)",
                    (meeting_id, user[0], role)
                )

        conn.commit()
        return jsonify({"success": True, "meeting_id": meeting_id, "redirect_url": f"/meeting?org_id={org_id}"})
