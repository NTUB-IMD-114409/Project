...
def add_topic_member():
    ...
    try:
        data = request.get_json()
        print("📥 收到新增議題成員資料：", data)

        topic_id = data.get("topic_id")
        user_id = data.get("user_id")
        role = data.get("role", "view")  # ✅ 新增這一行，預設是 view
        ...
        # 檢查是否已經是成員
        cursor.execute(
            "SELECT 1 FROM topic_participants WHERE topic_id = %s AND user_id = %s",
            (topic_id, user_id)
        )
        if cursor.fetchone():
