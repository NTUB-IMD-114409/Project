        ...
        while max_length >= min_length:
            try:
                ...
                if "exceed context window" in str(e):
                    max_length = int(max_length * step)
                    continue
                else:
                    return jsonify({"success": False, "message": str(e)}), 500
        # 全部都沒過
        return jsonify({"success": False, "message": "內容太長，請分段查詢！"}), 400
...
def delete_task(task_id):
    try:
        ...
        # 查任務的 assigner 是否為目前登入者
        cursor.execute("SELECT assigner_id FROM tasks WHERE id = %s", (task_id,))
...
def update_task():
    try:
        ...
        if str(row["assigner_id"]) != str(session_user_id):
