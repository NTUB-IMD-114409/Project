...
def task_page():
    ...
    return render_template("main.function/task.html", meeting_id=meeting_id)
...
# === 2. 語音轉文字 (Whisper 推論) ===
from punctuation_utils import restore_punctuation, restructure_text
...
def whisper_transcribe():
    try:
        ...
        if not raw_text:
            return jsonify({"error": "轉錄結果為空"}), 400

        # Step 2: 標點還原
        polished_text = restore_punctuation(raw_text)

        # Step 3: 通順重構
        final_text = restructure_text(polished_text)
