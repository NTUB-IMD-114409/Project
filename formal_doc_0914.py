...
def extract_todo_struct(item):
    return {
        "assignee": item.get("owner", ""),       # ← 這裡改成 owner
        "content": item.get("task", ""),         # ← 這裡改成 task
        "speaker": "",
...
def extract_todo_struct(item_text):
    import re
    match = re.search(r"負責[:：](.+?)；期限[:：](.+?)$", item_text)
    if match:
        return {
            "task": item_text.split("負責")[0].strip("：:；"),
            "owner": match.group(1).strip(),         # ✅ 改這
...
def generate_docx():
    ...
    try:
        ...
        # ✅ 處理成你模板要的 todo_items
        from blueprints.semantic_classifier import extract_todo_struct  # ⬅️ 確保有 import 這個
        ai_ctx["todo_items"] = [extract_todo_struct(item) for item in ai_ctx["ACTION_ITEM"]]
        ai_ctx["tpl_todo_items"] = ai_ctx["todo_items"]
        ...
        def extract_todo_struct(text):
            import re
            # 嘗試抓「請○○」→ assignee
            owner_match = re.search(r"請(\S+?)[:：\s]*", text)
            # 嘗試抓日期
            date_match = re.search(r"(\d{1,4}[/-]\d{1,2}[/-]\d{1,2}|[0-9]{1,2}[/-][0-9]{1,2})", text)
            return {
                "task": text.strip(),
                "assignee": owner_match.group(1) if owner_match else "（未標註）",
        ...
        # 轉換 todo 清單
        ai_ctx["todo_items"] = [extract_todo_struct(item) for item in ai_ctx.get("ACTION_ITEM", [])]
        ai_ctx["tpl_todo_items"] = ai_ctx["todo_items"]
