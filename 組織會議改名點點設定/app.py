...
def llama_summarize():
    try:
        data = request.get_json()
        file_path = data.get("file_path")        # 前端傳來的檔案路徑
        meeting_id = data.get("meeting_id")      # 前端傳來的會議 ID
        user_id = session["user"]["id"]          # 目前登入者 ID
        user_name = session["user"]["name"]      # 目前登入者姓名
        ...
        # 檢查是否已有該 meeting_id + file_path 的摘要
        cursor.execute(
            "SELECT id FROM summaries WHERE meeting_id = %s AND file_path = %s",
